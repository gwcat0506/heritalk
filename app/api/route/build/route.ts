...
import { buildRoute, NotEnoughStopsError, makeOriginPOI } from "@/lib/routeEngine";
...
export async function POST(req: Request) {
  try {
    const { startId, pois, startOrigin } = (await req.json()) as {
      startId?: string;
      pois: POI[];
      startOrigin?: LatLng;
    };
    ...
    // 현위치 출발 시 합성 origin을 시작점으로(거점 앞에 prepend).
